"""Disk-backed cache for "events from now onward" reads -- disk, not memory, since
Serverless sleep would wipe an in-memory cache between requests anyway."""
...
# Long enough to cover a burst of commands; short enough to bound staleness from
# edits made directly in Google Calendar.
CACHE_TTL_SECONDS = 300

# Covers the widest "from now" read (bulk actions' 365-day lookahead) so one fetch
# can serve everything narrower too.
...
def invalidate() -> None:
    """Called by every write in app.gcal.calendar -- drops the whole cache."""
    _write_entries([])
...
async def list_events_cached(credentials, calendar_ids: list[str], time_min: datetime, time_max: datetime) -> list[EventInfo]:
    """Cached alternative to list_events() for "start point onward" reads only --
    arbitrary single-day or recently-created lookups should call list_events() directly."""
